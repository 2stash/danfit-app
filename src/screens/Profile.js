  ...
  const Total = () => {
    ...
    for (let i = 0; i < allCompletedWorkouts.length; i++) {
      ...
      if(allCompletedWorkouts[i].workout.length > 1){
        ...
        totalBurpees += allCompletedWorkouts[i].workout[3].burpees * setNum || 0;
        totalPullups += allCompletedWorkouts[i].workout[4].pullups * setNum || 0;
      }

    }
    return (
      <View style={{marginTop:25}}>
        <Text style={{textAlign:'center', fontSize:24}}>LifeTime Pushups</Text>
        <Text style={{textAlign:'center',fontSize:28,marginTop:15
      }}>{totalPushups}</Text>

<Text style={{textAlign:'center', fontSize:24}}>LifeTime Situps</Text>
        <Text style={{textAlign:'center',fontSize:28,marginTop:15
      }}>{totalSitups}</Text>

<Text style={{textAlign:'center', fontSize:24}}>LifeTime Squats</Text>
        <Text style={{textAlign:'center',fontSize:28,marginTop:15
      }}>{totalSquats}</Text>

<Text style={{textAlign:'center', fontSize:24}}>LifeTime Burpees</Text>
        <Text style={{textAlign:'center',fontSize:28,marginTop:15
      }}>{totalBurpees}</Text>

<Text style={{textAlign:'center', fontSize:24}}>LifeTime Pullups</Text>
        <Text style={{textAlign:'center',fontSize:28,marginTop:15
      }}>{totalPullups}</Text>
      </View>
    );
  ...
  return (
    <View style={{ flex: 1 }}>
      {allCompletedWorkouts != undefined ?  <Total /> : (<View><Text>Loading data</Text></View>)}
